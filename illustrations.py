...
thumbs_folder = f'{cur_dir}\\resources\\media\\illustrations\\thumbs\\'
pages_folder = f'{cur_dir}\\resources\\media\\illustrations\\pages\\'
count = 0
...
list_of_imgs = os.listdir(f'{cur_dir}\\resources\\media\\illustrations')
list_of_imgs = [elm for elm in list_of_imgs if '.' in elm]

for i in range(len(list_of_imgs)):
    if '.' in list_of_imgs[i]:  
        print(f"checking image ({i+1}/{len(list_of_imgs)})", end="\r")
        if(i==0):
        ...
        f = open(f'{cur_dir}\\resources\\media\\illustrations\\pages\\{list_of_imgs[i]}.html', 'w')
        f.write(
f"""<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <div class="content">
        ...
        <div class="img-container">
            <img 
                class="full-img" 
                src="https://raw.githubusercontent.com/dogtiered/dogtiered.github.io/main/resources/media/illustrations/{list_of_imgs[i]}" 
            alt="why no here">
        </div>
...
<script>
        ...
        print(prev_img, next_img)
        

        image = Image.open(f'{cur_dir}\\resources\\media\\illustrations\\{list_of_imgs[i]}')
        image = ImageOps.exif_transpose(image)  #correct quirk with images 
                                                #not rotating correctly
        image.thumbnail((400,400))
        image.save(f'{cur_dir}\\resources\\media\\illustrations\\thumbs\\{list_of_imgs[i]}')
...
for img in list_of_imgs:
    html_string += f'<li><a href="../resources/media/illustrations/pages/{img}.html"><img src="../resources/media/illustrations/thumbs/{img}" alt="{img}"><div class="image-hover">{img.split(".")[0]}</div></a></li>'
